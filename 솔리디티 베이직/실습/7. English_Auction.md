...
## 목표
- NFT의 경매 시스템을 이해하고 직접 구축하는 방법을 익힌다.
- Solidity에서 다른 스마트 계약을 불러와 사용하는 방법을 배운다.
- 스마트 계약에서 이벤트를 사용하여 상태 변경을 추적하고 모니터링하는 방법을 학습한다.
- 스마트 계약의 보안 및 유효성 검사를 위한 require, revert, assert 사용법을 학습한다.
- ERC-721 인터페이스를 사용하여 NFT를 전송하고 소유권을 관리하는 방법을 익힌다.
- 스마트 계약을 통해 ETH를 송금하고 관리하는 방법을 이해한다.
- Remix IDE를 사용하여 스마트 계약을 배포하고 테스트하는 실습을 통해 실전 경험을 쌓는다.
...
### MyToken Mint
> MyToken contract를 컴파일 후 deploy한다. 아래와 같이 세팅 후 deploy하면 된다.<br><br>
...
> NFT를 민트한다. Deployed/Unpinned Contracts에서 deploy한 contract를 클릭한다. 그 후 아래와 같은 버튼들 중 safeMint 버튼 옆에 주소 적는 빈칸에 본인이 원하는 주소(본인의 주소를 추천한다)를 넣고 버튼을 클릭하면 된다.<br><br>
...
### English Auction 실행
> 1. EnglishAuction contract를 deploy 한다. 아래 이미지와 같이 만든 후 deploy 버튼을 누르면 된다. 이때 _NFT에는 위에서 deploy한 NFT 주소를, _NFTID에는 0(한개밖에 mint 안했기 때문에)을, _STARTINGBID에는 1ETH를 입력하기 위해서 1*10^18(wei 단위)을 넣어준다. <br><br>
...
> 2. EnglishAuction contract가 IERC721의 transferFrom함수를 사용하기 위해서는 해당 nft의 ID에 대한 approve가 되어 있어야 한다. MyToken contract의 approve 버튼 옆에 EnglishAuction contract의 주소를 입력 후 클릭한다.<br><br>
...
> 3. 먼저 auction을 시작하기 위히서 start버튼을 누른다. 이제 옥션이 시작된거다. 이때 사인하는 주소가 English Auction contract를 deploy한 주소와 동일해야 한다. <br><br>
> 4. 파란색 버튼들을 실행해보면 모두 정상 작동하는거를 확인할 수 있다. 이때 아직 아무도 bid를 하지 않았기 때문에 이 주소는 비어있다.<br><br>
...
> 6. Value를 2ETH로 바꾼 후 bid 버튼을 누른다. contract의 balance가 2ETH로 바뀌고 highestBid와 highestBidder가 바뀐 것을 확인할 수 있다.<br><br>
...
> 7. 주소와 bid하는 value를 바꿔가면 옥션을 진행한다.<br><br>
> 8. withdraw 함수는 해당 주소의 bid를 contract로부터 돌려받을 수 있다. 이때 highestBidder눈 withdraw 할 수 없다.<br><br>
> 10. 7일 이후에 end 함수를 누르면 옥션을 정지하고 highestbid만큼의 ETH가 seller의 주소로 전달된다. 
