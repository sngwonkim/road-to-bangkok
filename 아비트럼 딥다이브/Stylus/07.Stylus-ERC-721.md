...
NFT를 구축하고 배포하려면 몇가지 필요합니다.

기본적인 환경설정이 되어 있다는 가정하에 진행합니다.
...
위 파일은 다음과 같은 구성으로 되어있습니다.

`src/` 디렉토리에서 다음 소스 파일을 볼 수 있습니다. 각각 목적이 있으며, 각 파일 내에 관련 문서가 있습니다.

| 파일                               | 정보                                 |
| :--------------------------------- | :----------------------------------- |
| [`erc721.rs`](erc721.rs)           | [ERC-721 표준][erc721]을 구현합니다. |
| [`main.rs`](src/main.rs)           | 진입점을 정의합니다.                 |
| [`utils.rs`](src/utils.rs)         | 온체인 png 생성을 위한 유틸리티.     |
| [`print_art.rs`](src/print_art.rs) | 콘솔에 NFT를 인쇄합니다 👀           |
| [`art.rs`](art.rs)                 | **NFT를 그릴 수 있는 곳 😄**         |

### art.rs 중요 코드

generate_nft fn 을 통해서 아래와 같은 이미지가 그려집니다.
...
위 명령어를 치면 아래와 같이 png 파일이 draw 됩니다.
![alt text](image.png)

### 배포

아래 명령어를 통해 배포를 확인합니다.
...
//! 다른 계약 타입에 의해 상속되도록 설계되었습니다.
//!
//! [`Erc721Params`] 트레이트를 통해 [`Erc721`]의 동작을 구성할 수 있으며,
//! 이를 통해 이름, 심볼, 토큰 URI를 지정할 수 있습니다.
//!
//! 이 코드는 감사되지 않았으며 프로덕션 사용에 적합하지 않음을 유의하세요.
...
/// 곧 이 작업을 정리하기 위해 `#[derive(SolidityError)]`을 제공할 예정입니다.
...
// 이 메서드들은 외부 메서드에서 사용되는 도우미 역할을 합니다.
// 여기서 "pub"으로 표시된 메서드는 erc721 모듈 외부에서 사용 가능 (즉, lib.rs에서 호출 가능)합니다.
impl<T: Erc721Params> Erc721<T> {
    ...
    /// `token_id`를 `from`에서 `to`로 전송합니다.
    /// 이 함수는 `from`이 토큰의 소유자인지 확인하지만, `to`가 제로 주소가 아닌지 확인하지 않습니다. 이 함수는 소각에 사용할 수 있습니다.
    pub fn transfer(&mut self, token_id: U256, from: Address, to: Address) -> Result<(), Erc721Error> {
        ...
        // 현재로서는 스토리지를 다루는 것이 번거로울 수 있지만, 이는 곧 출시될 Stylus SDK 버전에서 변경될 것입니다.
    ...
    /// `to` 주소가 계약인 경우 `onERC721Received`를 호출합니다.
    /// 그렇지 않으면 아무 것도 하지 않습니다.
    ...
    /// 새로운 토큰을 발행하고 이를 안전하게 `to`로 전송합니다.
...
sol_interface! {
    /// `IERC721TokenReceiver`를 구현하는 다른 계약의 `onERC721Received` 메서드 호출을 허용합니다.
...
impl<T: Erc721Params> Erc721<T> {
    ...
    /// `to` 주소가 NFT를 받을 수 있는지 확인한 후 NFT를 전송합니다.
    ...
    /// [`safe_transfer_from`]과 동일하지만, 수신자에게 추가 데이터를 포함합니다.
    ///
    /// 참고: Rust는 동일한 이름의 메서드를 여러 개 허용하지 않기 때문에,
    /// `#[selector]` 매크로 속성을 사용하여 Solidity 오버로딩을 시뮬레이트합니다.
    ...
    /// 계정에 사용자의 NFT를 관리할 수 있는 권한을 부여합니다.
    ...
    /// 계정에 사용자의 모든 NFT를 관리할 수 있는 권한을 부여합니다.
