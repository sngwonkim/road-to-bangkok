...
`counter` 구조체와 유사하게 간단한 `은행계좌` 스마트 계약을 작성합니다. 6번까지 퀘스트를 완료하고 캡처해서 제출하세요!
...
// 1. 계좌 잔액을 저장하는 필드를 구현하세요
// 구조체의 이름은 BankAccount 입니다.

// 2. BankAccount 구조체를 구현하세요.
impl BankAccount {
    // 3. 저장소에서 현재 계좌 잔액을 가져옵니다.

    // 4. 저장소에 지정된 계좌 잔액을 설정합니다.

    // 5. 계좌에 금액을 입금합니다.

    // 6. 계좌에서 금액을 출금합니다. 잔액이 충분하지 않을 경우 에러를 반환합니다.
}
```
...
/// `BankAccount` 구조체의 메서드 구현입니다.
#[external]
impl BankAccount {
    /// 저장소에서 현재 계좌 잔액을 가져옵니다.
    pub fn get_balance(&self) -> Result<U256, Vec<u8>> {
        Ok(self.balance.get())
    }

    /// 저장소에 지정된 계좌 잔액을 설정합니다.
    ...
    /// 계좌에서 금액을 출금합니다. 잔액이 충분하지 않을 경우 에러를 반환합니다.
